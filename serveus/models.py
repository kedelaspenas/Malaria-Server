...
class User(db.Model, UserMixin):
	...
	"""
	@validates('username')
	def models_committed(self, key, value):
		try:
			password = os.urandom(4).encode('hex') 
			body = "Greetings, %s %s! You have successfully been registered to the system. Your temporary password is <b>%s</b>.<br><br>Below are your account details:<br><br>Username: %s<br>User type: %s<br>Contact number: %s<br>Email: %s<br><br>Please login and change your password. Thank you!" % (self.firstname, self.lastname, password, self.username, self.usertype, self.contact, self.email)
			self.password = User.hash_password(password)
			msg = Message('Outbreak Monitoring', sender="cvmig.group.23@gmail.com", recipients=['generic@mailinator.com',self.email])
			msg.html = body
			mail.send(msg)
		except:
			print 'test'
